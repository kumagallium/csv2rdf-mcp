...
| **M4 オントロジー可視化ギャラリー (見せ場)** | 生成済み全 dataset の Mermaid クラス図を一覧・拡大表示 (mermaid.js)。将来は TBox を WebVOWL でインタラクティブ表示 | 作成したオントロジーを視覚的に俯瞰 (ユーザ要望・デモ映え) |

**実装順 (見せ場優先・確定)**: **M0 → M1 → M4 → M2 → M3**。M4 (オントロジーギャラリー) を M2 (実取り込み) より前倒しし、「CSV を入れるとオントロジー図がギャラリーに並ぶ」デモ映えする状態に早く到達する。
